...
/**
  ...
  * cyby.server.example.BioS for aliases with often used type parameters.
  *
  ...
  */
case class Bio[F[_],ID,M,S,P,FS,CR,MO](
  id:           ID,
  value:        F[Double],
  method:       F[M],
  supplier:     F[S],
...
object Bio extends DataCmp {
  ...
  /**
    ...
    */
  type Cli       = Bio[Pure,Id,Link[Met.Id],Link[Sup.Id],Link[Pro.AccId],List[Fil.Cli],TimeStamp,EditInfo]
  ...
  implicit def eqI[F[_],ID,M,S,P,FS,CR,MO]: cats.Eq[Bio[F,ID,M,S,P,FS,CR,MO]] = cats.Eq.fromUniversalEquals
  implicit def decI[F[_]:D1,ID:D,M:D,S:D,P:D,FS:D,CR:D,MO:D]: D[Bio[F,ID,M,S,P,FS,CR,MO]] = deriveDecoder
  implicit def encI[F[_]:E1,ID:E,M:E,S:E,P:E,FS:E,CR:E,MO:E]: E[Bio[F,ID,M,S,P,FS,CR,MO]] = deriveEncoder
}
