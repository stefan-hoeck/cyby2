...
/**
  ...
  * cyby.server.example.StoS for aliases with often used type parameters.
  *
...
object Sto extends DataCmp {
  ...
  /**
    ...
    */
  type Cli       = Sto[Pure,Id,TimeStamp,EditInfo]

  val lblG = LabelledGeneric[Cli]
  val lbls@(id::name::comment::created::modified::HNil) = Keys[lblG.Repr].apply

  implicit def eqI[F[_],ID,CR,MO]: cats.Eq[Sto[F,ID,CR,MO]] = cats.Eq.fromUniversalEquals
  implicit def decI[F[_]:D1,ID:D,CR:D,MO:D]: D[Sto[F,ID,CR,MO]] = deriveDecoder
  implicit def encI[F[_]:E1,ID:E,CR:E,MO:E]: E[Sto[F,ID,CR,MO]] = deriveEncoder
}
