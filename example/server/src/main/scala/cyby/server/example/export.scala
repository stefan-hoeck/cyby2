...
import cyby.dat.example.{SubField ⇒ SField, ConField ⇒ CField, BioField ⇒ BField, _}
...
case class Export(coreSettings: CoreSettings)
  ...
  private def pairs(ss: List[Sub.Cli]) =
    ss.map(s ⇒ (s,s.structure.v.o)).collect{ case (s,Some(m)) ⇒ s -> m }

  private def act(ss: List[Sub.Cli], f: Format, fs: List[(ExportField,String)], fn: PathNIO) =
    f match {
  ...
  private def txt(fields: List[(ExportField,String)]): Sub.Cli ⇒ List[String] = s ⇒
    textLines(apply(field sub _ txt, field con _ txt, field bio _ txt, (_,d) ⇒ d.fold("")(_.toString), s, fields))

  private val structF = ExportSub(SubMol(DMol.Structure)).f

  private def sdf(fields: List[(ExportField,String)]): ((Sub.Cli,DMol)) ⇒ List[String] = {
